...
public abstract class CircuitGenerator {
	...
	//where is runKateCommitment?
	public String[] runKateCommitment() {

		String[] inputs = {"0", "0", "0"};

		try {
			Process p;
			p = Runtime.getRuntime().exec(new String[] {Config.COMMITMENT_EXEC, "10000", "10"});
			p.waitFor();
			System.out.println(
					"\n-------------------------------------- EVALUATION PROOF: KZG-10 ----------------------------------------");
			String line;
			String checkpoint = "This values are for jsnark";
			BufferedReader input = new BufferedReader(new InputStreamReader(p.getInputStream()));
			StringBuffer buf = new StringBuffer();
			...
			while ((line = input.readLine()) != null) {
				...
				if(line.equals(checkpoint)) {
					inputs[0] = input.readLine();
					inputs[1] = input.readLine();
					inputs[2] = input.readLine();
				}
			}
