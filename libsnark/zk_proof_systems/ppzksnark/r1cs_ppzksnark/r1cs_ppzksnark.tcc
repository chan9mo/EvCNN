                                                ...
                                                const r1cs_ppzksnark_auxiliary_input<ppT> &auxiliary_input)
{
	...
	/*
    for(size_t i =0;i<qap_wit.degree();i++){
		if(qap_wit.coefficients_for_H[i] != libff::Fr<ppT>::zero()){
                                                ...
                                                const r1cs_ppzksnark_auxiliary_input<ppT> &auxiliary_input)
{
...
#ifdef DEBUG
    assert(pk.constraint_convol_system.is_satisfied(primary_input, auxiliary_input));
#endif
    ...
    libff::enter_block("Compute the polynomial H");
    const qap_witness<libff::Fr<ppT> > qap_wit = r1cs_to_qap_witness_map(pk.constraint_convol_system, primary_input, auxiliary_input, d1, d2, d3);
    libff::leave_block("Compute the polynomial H");

#ifdef DEBUG
    const libff::Fr<ppT> t = libff::Fr<ppT>::random_element();
    qap_instance_evaluation<libff::Fr<ppT> > qap_inst = r1cs_to_qap_instance_map_with_evaluation(pk.constraint_convol_system, t);
    assert(qap_inst.is_satisfied(qap_wit));
#endif
