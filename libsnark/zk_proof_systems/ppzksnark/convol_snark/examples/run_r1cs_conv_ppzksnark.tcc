...
#ifndef RUN_R1CS_CONV_PPZKSNARK_TCC_
#define RUN_R1CS_CONV_PPZKSNARK_TCC_
                        ...
                        const bool test_serialization)
{
    ...
    r1cs_conv_ppzksnark_keypair<ppT> keypair = r1cs_conv_ppzksnark_generator<ppT>(example.constraint_system);

    printf("\n"); libff::print_indent(); libff::print_mem("after generator");

    libff::print_header("Preprocess verification key");
    r1cs_conv_ppzksnark_processed_verification_key<ppT> pvk = r1cs_conv_ppzksnark_verifier_process_vk<ppT>(keypair.vk);

	printf("\n");
    libff::print_header("R1CS GG-ppzkSNARK Prover");
    r1cs_conv_ppzksnark_proof<ppT> proof = r1cs_conv_ppzksnark_prover<ppT>(keypair.pk, example.primary_input, example.auxiliary_input);
    printf("\n"); libff::print_indent(); libff::print_mem("after prover");



    libff::print_header("R1CS GG-ppzkSNARK Verifier");
    const bool ans = r1cs_conv_ppzksnark_verifier_strong_IC<ppT>(keypair.vk, example.primary_input, proof);

    printf("\n"); libff::print_indent(); libff::print_mem("after verifier");
    printf("* The verification result is: %s\n", (ans ? "PASS" : "FAIL"));

    libff::print_header("R1CS GG-ppzkSNARK Online Verifier");
    const bool ans2 = r1cs_conv_ppzksnark_online_verifier_strong_IC<ppT>(pvk, example.primary_input, proof);
    printf("* The verification result is: %s\n", (ans2 ? "PASS" : "FAIL"));
    assert(ans == ans2);
                        ...
                        const bool test_serialization)
{
    ...
    size_t len = example.constraint_system.num_cminputs();
    size_t len2 = example2.constraint_system.num_cminputs();


    // std::cout<<"ex 1"<<" "<<len<<std::endl;
    // //0 1 2 3 4 5 6 7
    // //cm = 3
    // for(size_t i=example.primary_input.size() - len;i<example.primary_input.size();i++){
	//     // for(size_t j=0;j<example2.primary_input.size();j++){
	// 	//     if(example.primary_input[i] == example2.primary_input[j]){

    //     // 		std::cout<<i<<" : "<<example.primary_input[i].as_ulong()<<", "<<j<<" : "<<example2.primary_input[j].as_ulong()<<std::endl;
	// 	//     }
	//     // }
    //     std::cout<<i<<" : "<<example.primary_input[i].as_ulong()<<std::endl;
    // }
    // std::cout<<"ex 2"<<" "<<len2<<std::endl;
    // for(size_t i=example2.primary_input.size() - len2;i<example2.primary_input.size();i++){
    //     std::cout<<i<<", "<<example2.primary_input[i].as_ulong()<<std::endl;
    // }
    ...
    size_t cmstart_idx =  example.primary_input.size() - len;
    size_t cmstart_idx2 =  example2.primary_input.size() - len2;
    const accumulation_vector<libff::G1<ppT> > cx = pvk.gamma_ABC_g1.template accumulate_chunk<libff::Fr<ppT> >(example.primary_input.begin()+(cmstart_idx), example.primary_input.end() , 0);
    const accumulation_vector<libff::G1<ppT> > cx2 = pvk2.gamma_ABC_g1.template accumulate_chunk<libff::Fr<ppT> >(example2.primary_input.begin()+(cmstart_idx2), example2.primary_input.end(), 0);

    // libff::G1<ppT> ccx = pvk.gamma_ABC_g1.first;

    // std::cout<<"ex 1"<<std::endl;
    // for(size_t i=0;i<len;i++){
    //     ccx = ccx + (example.primary_input[i+cmstart_idx] * pvk.gamma_ABC_g1.rest.values[i]);
    //     std::cout<<i<<", "<<example.primary_input[i].as_ulong()<<std::endl;
    ...
    //     ccx2 = ccx2 + (example2.primary_input[i+cmstart_idx2] * pvk2.gamma_ABC_g1.rest.values[i]);
    //     std::cout<<i<<", "<<example2.primary_input[i].as_ulong()<<std::endl;
    // }
    ...
    libff::print_header("LEGO Prover");
    libff::enter_block("Call to LEGO_prover");
    r1cs_primary_input<libff::Fr<ppT>> cminput(example.primary_input.begin()+cmstart_idx, example.primary_input.end());
    r1cs_legosnark_proof<ppT> lego_proof = r1cs_legosnark_prover<ppT>(legokey.pk, cminput, len);//example.primary_input);
    libff::leave_block("Call to LEGO_prover");

    libff::print_header("LEGO Verifier");
    libff::enter_block("Call to LEGO_verifier");
    const bool legoans = r1cs_legosnark_verifier<ppT>(legokey.vk,cx.first, cx2.first, lego_proof); //cx.first, cx2.first, lego_proof);
    printf("* The verification result is: %s\n", (legoans ? "PASS" : "FAIL"));
    libff::leave_block("Call to LEGO_verifier");
