...
#ifndef MVR1CS_TO_QAP_TCC_
#define MVR1CS_TO_QAP_TCC_
...
qap_instance<FieldT> mvr1cs_to_qap_instance_map(const mvr1cs_constraint_system<FieldT> &cs)
{
    libff::enter_block("Call to mvr1cs_to_qap_instance_map");
    ...
    libff::leave_block("Call to mvr1cs_to_qap_instance_map");
 ...
 */
template<typename FieldT>
qap_instance_evaluation<FieldT> mvr1cs_to_qap_instance_map_with_evaluation(const mvr1cs_constraint_system<FieldT> &cs,
                                                                         const FieldT &t)
{
    


    libff::enter_block("Call to mvr1cs_to_qap_instance_map_with_evaluation");
    ...
    for (size_t i = 0; i < cs.num_constraints(); ++i)
    {
        ...
        for(size_T j=0;j<cs.constraints[i].a2.terms.size();j++)
        {
            At[cs.constraints[i].a2.terms[j].index] +=
                u[i]*cs.constraints[i].a2.terms[j].coeff;
        }
        //hellohello
    ...
    libff::leave_block("Call to mvr1cs_to_qap_instance_map_with_evaluation");
 ...
 */
template<typename FieldT>
qap_witness<FieldT> mvr1cs_to_qap_witness_map(const mvr1cs_constraint_system<FieldT> &cs,
                                            const mvr1cs_primary_input<FieldT> &primary_input,
                                            const mvr1cs_auxiliary_input<FieldT> &auxiliary_input,
                                            const FieldT &d1,
                                            const FieldT &d2,
                                            const FieldT &d3)
{
    libff::enter_block("Call to mvr1cs_to_qap_witness_map");
    ...
    mvr1cs_variable_assignment<FieldT> full_variable_assignment = primary_input;
...
#pragma omp parallel for
    ...
    libff::leave_block("Call to mvr1cs_to_qap_witness_map");
