                                                                         ...
                                                                         const FieldT &t)
{
    ...
    const std::vector<FieldT> u = domain2->evaluate_all_lagrange_polynomials(t);
    /**
     * add and process the constraints
     *     input_i * 0 = 0
     * to ensure soundness of input consistency
    ...
    libff::leave_block("Call to r1cs_to_mqap_instance_map_with_evaluation");

    return mqap_instance_evaluation<FieldT>(domain,
                                           cs.num_variables(),
                                           domain->m,
										   domain2->m,
										   domain4->m,
                                            ...
                                            const FieldT &d3)
{
    ...
    std::vector<FieldT> aA(domain2->m, FieldT::zero()), aB(domain2->m, FieldT::zero());
    ...
    for (size_t i = 0; i < cs.num_constraints(); ++i)
    {
		...
		///TODO if constraint has unkown 's' ,then aA gonna what value
    }
