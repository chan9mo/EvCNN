...
mr1cs_constraint<FieldT>::mr1cs_constraint(const linear_combination<FieldT> &a,
...
template<typename FieldT>
mr1cs_constraint<FieldT>::mr1cs_constraint(const std::initializer_list<linear_combination<FieldT> > &A,
                                         ...
                                         const std::initializer_list<linear_combination<FieldT> > &C2)
{
    ...
    for (auto lc_A2 : A2)
    {
        a2.terms.insert(a2.terms.end(), lc_A2.terms.begin(), lc_A2.terms.end());
    }
    for (auto lc_B2 : B2)
    {
        b2.terms.insert(b2.terms.end(), lc_B2.terms.begin(), lc_B2.terms.end());
    }
    for (auto lc_C2 : C2)
    {
        c2.terms.insert(c2.terms.end(), lc_C2.terms.begin(), lc_C2.terms.end());
    }
}

template<typename FieldT>
bool mr1cs_constraint<FieldT>::operator==(const mr1cs_constraint<FieldT> &other) const
{
...
template<typename FieldT>
std::ostream& operator<<(std::ostream &out, const mr1cs_constraint<FieldT> &c)
{
    out << c.a;
    out << c.b;
    out << c.c;
...
template<typename FieldT>
std::istream& operator>>(std::istream &in, mr1cs_constraint<FieldT> &c)
{
    in >> c.a;
    in >> c.b;
    in >> c.c;
...
template<typename FieldT>
size_t mr1cs_constraint_system<FieldT>::num_inputs() const
{
    return primary_input_size;
}

template<typename FieldT>
size_t mr1cs_constraint_system<FieldT>::num_variables() const
{
    return primary_input_size + auxiliary_input_size;
}

template<typename FieldT>
size_t mr1cs_constraint_system<FieldT>::num_constraints() const
{
    return constraints.size();
}

template<typename FieldT>
size_t mr1cs_constraint_system<FieldT>::num_convol() const
{
    return this.convol_size;
}

template<typename FieldT>
size_t mr1cs_constraint_system<FieldT>::num_convol_outputs() const
{
    return this.convol_outputs_size;
}

template<typename FieldT>
bool mr1cs_constraint_system<FieldT>::is_valid() const
{
    ...
    for (size_t c = 0; c < constraints.size(); ++c)
    {
        if (!(constraints[c].a.is_valid(this->num_variables()) &&
              ...
              constraints[c].a2.is_valid(this->num_variables()) &&
              constraints[c].b2.is_valid(this->num_variables()) &&
              constraints[c].c2.is_valid(this->num_variables())))
        {
            return false;
        }
    }
...
template<typename FieldT>
void dump_mr1cs_constraint(const mr1cs_constraint<FieldT> &constraint,
                          const mr1cs_variable_assignment<FieldT> &full_variable_assignment,
                          const std::map<size_t, std::string> &variable_annotations)
{
    ...
    printf("terms for a2:\n"); constraint.a2.print_with_assignment(full_variable_assignment, variable_annotations);
    printf("terms for b2:\n"); constraint.b2.print_with_assignment(full_variable_assignment, variable_annotations);
    printf("terms for c2:\n"); constraint.c2.print_with_assignment(full_variable_assignment, variable_annotations);
}

///TODO make satisfy function
template<typename FieldT>
bool mr1cs_constraint_system<FieldT>::is_satisfied(const mr1cs_primary_input<FieldT> &primary_input,
                                                  const mr1cs_auxiliary_input<FieldT> &auxiliary_input) const
{
    ...
    for (size_t c = 0; c < constraints.size(); ++c)
    {
        ...
        if (!(ares*bres == cres))
        {
            ...
            dump_mr1cs_constraint(constraints[c], full_variable_assignment, variable_annotations);
#endif // DEBUG
            return false;
        }
    }
...
template<typename FieldT>
void mr1cs_constraint_system<FieldT>::add_constraint(const mr1cs_constraint<FieldT> &c)
{
    constraints.emplace_back(c);
}

template<typename FieldT>
void mr1cs_constraint_system<FieldT>::add_constraint(const mr1cs_constraint<FieldT> &c, const std::string &annotation)
{
#ifdef DEBUG
...
///TODO how it works
template<typename FieldT>
void mr1cs_constraint_system<FieldT>::swap_AB_if_beneficial()
{
    libff::enter_block("Call to mr1cs_constraint_system::swap_AB_if_beneficial");
    ...
    libff::leave_block("Call to mr1cs_constraint_system::swap_AB_if_beneficial");
}

template<typename FieldT>
bool mr1cs_constraint_system<FieldT>::operator==(const mr1cs_constraint_system<FieldT> &other) const
{
...
template<typename FieldT>
std::ostream& operator<<(std::ostream &out, const mr1cs_constraint_system<FieldT> &cs)
{
    out << cs.primary_input_size << "\n";
    out << cs.auxiliary_input_size << "\n";
    out << cs.convol_outputs_size << "\n";
    out << cs.convol_size << "\n";

    out << cs.num_constraints() << "\n";
    for (const mr1cs_constraint<FieldT>& c : cs.constraints)
    {
        out << c;
    }
...
//TODO change to mr1cs
template<typename FieldT>
std::istream& operator>>(std::istream &in, mr1cs_constraint_system<FieldT> &cs)
{
...
//TODO change to mr1cs
template<typename FieldT>
void mr1cs_constraint_system<FieldT>::report_linear_constraint_statistics() const
{
#ifdef DEBUG
